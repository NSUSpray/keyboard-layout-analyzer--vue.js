...
<template>
  ...
  <p><strong>Flaw 1</strong>: no consideration is taken into account of the type of movement.</p>
  ...
  <p><strong><u>Fixes:</u></strong> This version of the app applies fixes to the algorithm to address all these flaws.</p>
  <ul>
    <li>The simple distance calculation is replaced by a “distance penalty”.</li>
    <li>The penalty is finger-dependent for actions that require a simple inward/outward curling of the finger, for example to reach upward with the middle finger from <em>K to I</em> (in Qwerty).</li>
    <li>The penalty is larger for horizontal movements, recognising that this component requires the whole hand to move or for fingers to splay out awkwardly, for example the motion of the index finger from <em>F to G</em> (in Qwerty).</li>
    <li>The coordinate system for movement vectors is rotated to align with the angle of approach of the hands. This currently set to a 10° angle, and is applied clockwise for the left hand, anticlockwise for the right hand.</li>
    <li>Fitts’s Law is incorporated in this version of the analyzer so both horizontal and vertical components are appropriately scaled.</li>
  </ul>
  ...
  <p><strong><u>Fix:</u></strong> A simpler effort calculation is employed based on the finger weightings in the table above, representing the relative strength of each finger.
  ...
  <p><strong><u>Fix:</u></strong> No fix needed. The weighting of this element of the calculation is increased from 17% to 30%.</p>
